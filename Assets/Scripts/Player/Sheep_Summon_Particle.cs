...
public class Sheep_Summon_Particle : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(Vector3.Distance(transform.position, spawnPoint) <= 0.5 && canSpawn)
        {
            ...
            if (rand <= player.summonBlackSheepPercent || (sheepType == SheepTypes.BUILD && rand <= player.summonBlackSheepPercent + WorldState.instance.passiveValues.builderCorruptChance) || player.gothMode.gothMode) 
